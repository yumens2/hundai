...
//    @Description("게임 재시작 여부를 선택")
    private static int checkReStart() {
        System.out.println("Want to restart? 1(Yes), 2(No): ");
        Scanner scanner = new Scanner(System.in);
        int rst = scanner.nextInt();

        if (rst != 1 && rst != 2) throw new IllegalArgumentException();
        else{
            return rst;
        }
    }
    ...
    private static Boolean showResult(ArrayList<Integer> userNumArray, ArrayList<Integer> randNumArray) {
        int strike = 0;
        int ball = 0;
        // 1. 스트라이크 개수 계산
