...
|  분류   | 이름               | 설명                             | 기능과 설명                               |
|-------|------------------|--------------------------------|--------------------------------------|
| 인터페이스 | ValueMaker       | 1~9중 세자리 숫자를 제공하는 역할을 하는 인터페이스 | public String makeNum();             |
| 클래스   | RandomValueMaker | ValueMaker를 구현한 클래스            | public String makeNum() {} : 랜덤하게 제공 |
### 🛠️ 값 판별 관련
|  분류   | 이름                   | 설명                          | 기능과 설명                                                         |
|-------|----------------------|-----------------------------|----------------------------------------------------------------|
| 인터페이스 | `ValueChecker`         | 입력값과 문제값을 비교하는 역할을 하는 인터페이스 | `public abstract String determineTheValue(String inputValue)`;   |
| 클래스   | `StandardValueChecker` | `ValueChecker` 구현한 클래스        | `public class StandardValueChecker implements ValueChecker{}` 구현 |


### 🛠️ 문제 맞추기 관련
|  분류   | 이름                | 설명                    | 기능과 설명                                          |
|-------|-------------------|-----------------------|-------------------------------------------------|
| 인터페이스 | `Participant`     | 문제를 맞추는 역할을 하는 인터페이스  | `public String inputValue();` : 입력값을 제공한다.      |
| 클래스   | `UserParticipant` | `Participant` 구현한 클래스 | `public String inputValue();`구현 : 콘솔을 통해 입력받는다. |

### 🛠️ 게임 구성 관련
| 분류     | 이름                    | 설명                    | 기능과 설명                                          |
|:-------|:----------------------|-----------------------|-------------------------------------------------|
| 클래스    | `ConfigurationObject` | 문제 출제 구현체와, 입 심사 구현체, 참여 구현체를 주입해준다.   | `getValueMaker;` : 문제 출제 구현체 제공. <br>`getValueChecker;` : 입력 심사 구현체 제공. <br>`getParticipant;` : 참여자 구현체 제공. <br>       |

### 🛠️ 게임 진행 관련
#### 개임 진행 인터페이스 : `GameManager`
|  분류   | 이름                | 설명                    | 기능과 설명 |
|-------|-------------------|-----------------------|--------|
| 클래스 | `GameManager`| 게임을 진행하는 플랫폼 역할을 한다. | -      |


### 느낀점
```
왜 이렇게 주입주입!! 하나 했는데, 테스트 코드를 짜면서 정말 많이 느꼈다.
좋은 코드는 테스트 코드를 짜기 쉬운 코드라고 생각한다.

메서드가 하나의 기능만 가지고 있으면 이해하기 쉬워서 테스트코드를 짜기 쉽다.
또한 주입을 통한 메서드는 테스트코드를 짜기 쉽다.!!

좋은 코드인지는 모르겠지만...!!

오늘까지만 하는게 아니고 다른 쿠키들의 코드를 보고나서도 조금 더 리팩토링 해봐야겠다!
```
